...
(*let cons_sexprs sexp1 sexp2 = 
match sexp1 with
| Pair(car, Nil) -> Pair(car, sexp2)
| Pair()
...
| Pair(Symbol("unquote-slicing"), Pair(car, cdr)) ->
  (match car, cdr with
  | Pair(Symbol("unquote-slicing"), sexpr), cdr -> *)
